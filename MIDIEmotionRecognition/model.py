...
def combine_sequences(melody, harmony):
    ...
    harmony_df = pd.DataFrame(harmony, columns=["chord_pitch0", "chord_pitch1", "chord_pitch2", "chord_pitch3",
                                                "chord_pitch4", "chord_pitch5", "chord_pitch6", "chord_pitch7",
                                                "chord_step", "chord_duration", "chord_function", "chord_function_accidental",
                                                "chord_quality", "chord_inversion"])
    harmony_df["offsets"] = calculate_offsets(harmony_df["chord_step"])
    combined = pd.merge(melody_df, harmony_df, how="outer", on="offsets").sort_values("offsets").drop_duplicates()
...
if __name__ == "__main__":
    ...
    train_features = pad_data(combine_all(train_melody, train_harmony), 1172)
    ...
    test_features = pad_data(combine_all(test_melody, test_harmony), 1172)
